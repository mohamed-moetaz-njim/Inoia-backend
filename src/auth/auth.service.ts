import { Injectable, ForbiddenException, UnauthorizedException, BadRequestException, ConflictException } from '@nestjs/common';
...
export class AuthService {
  ...
  async verifyEmail(email: string, token: string) {
      ...
      if (!user || !user.verificationToken) throw new BadRequestException('Invalid request');
    ...
    const [at, rt] = await Promise.all([
      ...
      this.jwtService.signAsync(
        { sub: userId, username, role },
        {
          ...
          expiresIn: this.configService.getOrThrow<string>('REFRESH_EXPIRES_IN') as any,
        },
      ),
    ]);
  ...
  async resetPassword(email: string, token: string, newPassword: string) {
      const user = await this.usersService.findOne({ email });
      if (!user || !user.resetToken) throw new BadRequestException('Invalid request');
      ...
      const now = new Date();
      const tokenAge = now.getTime() - user.updatedAt.getTime();
      if (tokenAge > 15 * 60 * 1000) { // 15 mins
          // Invalidate
           await this.usersService.update({
              where: { id: user.id },
