...
describe('ForumService', () => {
  ...
  describe('findOnePost', () => {
    ...
    it('should throw NotFoundException if post not found', async () => {
      ...
      await expect(service.findOnePost('post1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw NotFoundException if post is deleted and user is not owner', async () => {
      ...
      await expect(service.findOnePost('post1', 'user2')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('createPost', () => {
    it('should create a post', async () => {
      ...
      const result = await service.createPost('user1', { title: 'T', content: 'C' });
  ...
  describe('updatePost', () => {
    it('should update post if user is owner', async () => {
      ...
      const result = await service.updatePost('user1', 'post1', { content: 'Updated' });

      expect(result.content).toBe('Updated');
    });

    it('should throw NotFoundException if post deleted', async () => {
        prisma.post.findUnique.mockResolvedValue({ ...mockPost, deletedAt: new Date() });
        await expect(service.updatePost('user1', 'post1', { content: 'U' })).rejects.toThrow(NotFoundException);
    });

    it('should throw NotFoundException if user not owner', async () => {
        prisma.post.findUnique.mockResolvedValue(mockPost);
        await expect(service.updatePost('user2', 'post1', { content: 'U' })).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('deletePost', () => {
    ...
    it('should throw NotFoundException if user not owner', async () => {
        prisma.post.findUnique.mockResolvedValue(mockPost);
        await expect(service.deletePost('user2', 'post1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('createComment', () => {
      ...
      it('should throw NotFoundException if post not found', async () => {
          prisma.post.findUnique.mockResolvedValue(null);
          await expect(service.createComment('u', 'p', { content: 'c' })).rejects.toThrow(NotFoundException);
      });
  });
