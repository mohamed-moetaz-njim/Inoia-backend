...
describe('Authorization (e2e)', () => {
  ...
  describe('Error Semantics', () => {
      ...
      it('Deleted user access returns 404 (Resource Privacy)', async () => {
        ...
        // 1. Restore for login
        await prisma.user.update({ where: { id: student.id }, data: { deletedAt: null } });
  ...
  describe('Throttling (Basic Check)', () => {
      it('Refresh token invalidated on password reset', async () => {
          ...
          const resetReq = await prisma.user.update({
              where: { id: user.id },
              data: { resetToken: await argon2.hash('token123'), updatedAt: new Date() }
          });

          // 3. Reset Password
          await request(app.getHttpServer())
              .post('/auth/reset-password')
              .send({ email: 'reset@test.com', token: 'token123', newPassword: 'newpassword123' })
              .expect(200);
