  ...
  describe('POST /forum/posts', () => {
    ...
    it('Student can create post', async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .send({
          title: 'Student Post',
          content: 'This is a student post content',
        })
        .expect(201);
    ...
    it('Therapist can create post and exposes profile', async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${token}`)
        .send({
          title: 'Therapist Post',
          content: 'This is a therapist post content',
        })
        .expect(201);

      expect(res.body.title).toBe('Therapist Post');
      expect(res.body.author.therapistProfile).toBeDefined();
      expect(res.body.author.therapistProfile.certificationReference).toBe(
        'CERT-123',
      );
    });
  });
