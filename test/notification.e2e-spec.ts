...
describe('Notification (e2e)', () => {
  ...
  describe('Notifications Flow', () => {
    it('should notify post author when someone comments', async () => {
      const { user: author, token: authorToken } = await createAuthenticatedUser('Author', 'author@test.com');
      const { user: commenter, token: commenterToken } = await createAuthenticatedUser('Commenter', 'commenter@test.com');
      ...
      expect(res.body.data[0].content).toContain('@Commenter commented on your post');
    ...
    it('should notify mentioned user', async () => {
      const { user: author, token: authorToken } = await createAuthenticatedUser('Author', 'author@test.com');
      const { user: commenter, token: commenterToken } = await createAuthenticatedUser('Commenter', 'commenter@test.com');
      const { user: mentioned, token: mentionedToken } = await createAuthenticatedUser('Mentioned', 'mentioned@test.com');
